...
<template>
  <UPage>
    ...
    >
      ...
      <template #title>
        Full Stack with
        <span class="text-primary-500"> MongoDB </span>
      </template>
      <template #description>
        learn how to build a blogging website with authentication with Nuxt 3,
        Nuxt UI and Mongoose
      </template>
      <template #links>
        <UButton
          size="xl"
    ...
    <UModal v-model="deleteModal" prevent-close>
      <UCard>
        <div>Are you sure you want to delete "{{ selected?.name }}" ?</div>
