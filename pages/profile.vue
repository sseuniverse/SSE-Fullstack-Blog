...
const view = useSessionStorage<"grid" | "list">(
...
const formatted = computed(() => {
  return data.value
    ...
    .sort(
      (a, b) =>
        sortingFactors[sortingOption.value](a, b) * (order.value ? 1 : -1)
    );
});
...
<template>
  <UPage>
    <UPageBody class="relative mt-20">
            ...
            <div>
              <UButtonGroup>
                <UButton
                  ...
                  color="white"
                  :icon="
                    !order ? 'i-ph-sort-ascending' : 'i-ph-sort-descending'
                  "
                  @click="order = !order"
                />
                <USelectMenu
                ...
                <UButton
                  color="gray"
                  :icon="
                    view === 'grid' ? 'i-ph-grid-four-duotone' : 'i-ph-list'
                  "
                  @click="changeView"
                />
              </UButtonGroup>
      ...
      <div
        class="absolute -top-14 left-1/2 transform -translate-x-1/2 flex flex-col items-center gap-2"
      >
        <UAvatar
          size="3xl"
          :src="`https://unavatar.io/gravatar/${user.email}`"
